    ...
    return (
        <>
            <header className="w-full h-16 bg-white border-b">
                <div className="w m-auto h-full flex items-center justify-between lg:w" style={{"--w": "94%","--w-lg": "80%"}}>
                    <div>
                        <Link href="/dashboard" className="text-lg font-medium">Andes Analytics</Link>
                        <span className="text-xs text-muted ml-3">Panel interno</span>
                    </div>
                    <div className="flex items-center gap-sm">
                        <Link href="/" className="btn btn-sm btn-secondary">Ver web pública</Link>
            ...
            <main className="w-full bg-surface min-h-screen py-8">
                <div className="w m-auto flex flex-col gap-lg lg:w" style={{"--w": "94%", "--w-lg": "80%"}}>
                    <section className="w-full flex flex-col gap-md md:flex-row md:items-end md:justify-between">
                        <div>
                        ...
                        <div className="field">
                            ...
                            <select id="period" className="input" value={days} onChange={event => setDays(Number(event.target.value))} disabled={loading}>
                        ...
                        <>
                            <section className="w-full grid grid-cols-1 gap-md sm:grid-cols-2 xl:grid-cols-4">
                                {metrics.map(item => (
                                    <article key={item.label} className="card card-kpi">
                                        ...
                                        <p className="card-kpi-value">{loading ? "—" : item.value}</p>
                            ...
                            <section className="w-full grid grid-cols-1 gap-md lg:grid-cols-3">
                                ...
                                <article className="card lg:col-span-2">
                                    <div className="card-header">
                                        <div>
                                            <h2 className="card-title">Conversión de Pulso Ciudadano</h2>
                                            <p className="card-description">Usuarios que iniciaron y finalizaron el cuestionario.</p>
                                        </div>
                                    ...
                                    <div className="card card-subtle mt-md">
                                        ...
                                        <p className="text-sm text-muted mt-2">De cada 100 personas que comienzan Pulso Ciudadano, aproximadamente{" "} <strong>{completionRate}</strong>{" "} terminan el cuestionario.</p>
                                    </div>
                                </article>
                                <article className="card">
                                    <div className="card-header">
                                        <div>
                                            ...
                                            <p className="card-description">Métricas generales de uso.</p>
                                        </div>
                                    </div>
                            ...
                            <section className="w-full grid grid-cols-1 gap-md lg:grid-cols-2">
                                ...
                                <article className="card">
                                    <div className="card-header">
                                        <div>
                                            <h2 className="card-title">Fuentes de tráfico</h2>
                                            <p className="card-description">Desde dónde llegan los visitantes.</p>
                                        </div>
                                    </div>
                                    {topSources.length > 0 ? (
                                        <div className="w-full flex flex-col gap-md">
                                            {topSources.map(item => {
                                                ...
                                                return (
                                                    <div key={item.source} className="w-full flex flex-col gap-xs">
                                ...
                                <article className="card">
                                    ...
                                    <div className="card-header">
                                        <div>
                                            <h2 className="card-title">Páginas más visitadas</h2>
                                            <p className="card-description">Contenidos con mayor tráfico.</p>
                                        </div>
                                    </div>
                                    {topPages.length > 0 ? (
                                        <div className="w-full flex flex-col gap-md">
                                            {topPages.map(item => {
                                                ...
                                                return (
                                                    <div key={item.page_path} className="w-full flex flex-col gap-xs">
                                                        <div className="w-full flex items-center justify-between gap-md">
